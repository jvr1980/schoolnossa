#!/usr/bin/env python3
"""
Enriches Berlin primary school (Grundschule) data with nearby Points of Interest (POI) using Google Places API (New).
...
POI Categories (optimized for primary school parents):
- Supermarkets (grocery shopping)
- Restaurants (nearby dining options)
- Bakeries/Cafes (breakfast/snacks)
- Kitas (preschools - for siblings)
- Secondary Schools (for future reference)
- Parks/Playgrounds (after-school activities)
...
CHECKPOINT_FILE = DATA_DIR / "intermediate" / "poi_enrichment_checkpoint.json"
PROGRESS_FILE = DATA_DIR / "intermediate" / "poi_enrichment_progress.csv"

# Google Places API (New) endpoint
NEARBY_SEARCH_URL = "https://places.googleapis.com/v1/places:searchNearby"

# POI Categories to search using Places API (New) types
# Optimized for primary school parents/families
POI_CATEGORIES = {
    "supermarket": {
...
DETAILED_CATEGORIES = ["kita", "secondary_school", "supermarket", "bakery_cafe", "park_playground"]
...
def main():
    """Main function to enrich all primary schools with POI data."""
    print("="*70)
    print("ENRICHING GRUNDSCHULE DATA WITH NEARBY POINTS OF INTEREST")
    print("="*70)
    ...
    if not SCHOOLS_FILE.exists():
        ...
        print("Please run the transit enrichment script first (which includes geocoding)")
        return
    ...
    estimated_calls = total_to_process * len(POI_CATEGORIES) * 1.5  # 1.5x for text searches
    estimated_cost = estimated_calls * 0.035

    print(f"\nEstimated API calls: ~{int(estimated_calls)}")
