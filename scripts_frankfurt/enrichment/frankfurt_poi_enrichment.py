...
CHECKPOINT_FILE = INTERMEDIATE_DIR / "frankfurt_poi_enrichment_checkpoint.json"
    ...
    def get_stats(self):
        with self.lock:
            ...
            return {"processed": self.processed, "total": self.total,
                    "errors": self.errors, "api_calls": self.api_calls,
                    "elapsed_s": elapsed}
...
def fetch_nearby_places(lat, lon, included_types, text_query=None, radius=SEARCH_RADIUS_M):
    ...
    if text_query and len(results) < 5:
        ...
        for _ in range(MAX_RETRIES):
            try:
                resp = requests.post(TEXT_SEARCH_URL, headers=headers2, json=body2, timeout=15)
                api_calls += 1
                if resp.status_code == 200:
                    results.extend(resp.json().get("places", []))
                    break
...
def enrich_schools(school_type="secondary"):
    ...
    INTERMEDIATE_DIR.mkdir(parents=True, exist_ok=True)
    out = INTERMEDIATE_DIR / f"frankfurt_{school_type}_schools_with_pois.csv"
    ...
    print(f"\n{'='*70}\nFRANKFURT POI ENRICHMENT ({school_type.upper()}) - COMPLETE\n{'='*70}")
...
def main():
    logger.info("=" * 60)
    logger.info("Starting Frankfurt POI Enrichment (Google Places API)")
    logger.info("=" * 60)
