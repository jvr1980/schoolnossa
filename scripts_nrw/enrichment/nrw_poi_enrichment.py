...
# Checkpoint
CHECKPOINT_FILE = INTERMEDIATE_DIR / "nrw_poi_enrichment_checkpoint.json"
...
POI_CATEGORIES = {
    ...
    "secondary_school": {
        ...
        "textQuery": "Gesamtschule Gymnasium Realschule Sekundarschule"
    }
}
...
def fetch_nearby_places(lat: float, lon: float, included_types: List[str],
                        text_query: Optional[str] = None, radius: int = SEARCH_RADIUS_M) -> Tuple[List[dict], int]:
...
def enrich_schools(school_type: str = "secondary") -> pd.DataFrame:
    """Run POI enrichment for a school type."""
    logger.info(f"Running POI enrichment for {school_type} schools...")

    if not GOOGLE_PLACES_API_KEY:
        logger.error("GOOGLE_PLACES_API_KEY not set! Set it in .env file.")
        return pd.DataFrame()

    # Find input file
    for parent_dir in [INTERMEDIATE_DIR, RAW_DIR]:
        for pattern in [
            f"nrw_{school_type}_schools_with_crime.csv",
            f"nrw_{school_type}_schools_with_transit.csv",
            f"nrw_{school_type}_schools_with_traffic.csv",
            f"nrw_{school_type}_schools.csv",
        ]:
            input_file = parent_dir / pattern
        ...
        logger.error(f"No {school_type} school data found")
        return pd.DataFrame()
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for idx, row in schools_to_process:
            future = executor.submit(enrich_school, idx, row['latitude'], row['longitude'], row.get('schulname', ''))
            futures[future] = idx
    ...
    output_file = INTERMEDIATE_DIR / f"nrw_{school_type}_schools_with_pois.csv"
    ...
    print(f"\n{'=' * 70}")
    print(f"NRW POI ENRICHMENT ({school_type.upper()}) - COMPLETE")
    print(f"{'=' * 70}")
...
def main():
    """Main entry point."""
    logger.info("=" * 60)
    logger.info("Starting NRW POI Enrichment (Google Places API)")
    logger.info("=" * 60)
