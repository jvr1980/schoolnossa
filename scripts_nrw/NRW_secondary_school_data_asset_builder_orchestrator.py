#!/usr/bin/env python3
"""
NRW Secondary School Data Asset Builder Orchestrator

This orchestrator runs the complete NRW secondary school data pipeline
...
Phase 5b: Enrich with Anmeldezahlen / application numbers (Düsseldorf only)
...
Usage:
    python NRW_secondary_school_data_asset_builder_orchestrator.py
    python NRW_secondary_school_data_asset_builder_orchestrator.py --phases 1,2,3
    python NRW_secondary_school_data_asset_builder_orchestrator.py --skip-embeddings
...
def run_phase_1():
    """Phase 1: Download and parse NRW school master data."""
    logger.info("=" * 60)
...
def run_full_pipeline(phases: list = None, skip_embeddings: bool = False):
    ...
    logger.info("=" * 70)
    logger.info("NRW SECONDARY SCHOOL DATA ASSET BUILDER - STARTING")
    logger.info(f"Target: Cologne & Düsseldorf secondary schools")
    ...
    for phase_num in phases_to_run:
        if phase_num in available_phases:
            ...
            try:
                ...
                # For phase 1, stop pipeline if it fails
                if phase_num == 1:
    ...
    print("\n" + "=" * 70)
    print("NRW SECONDARY SCHOOL DATA ASSET BUILDER - COMPLETE")
    print("=" * 70)
    print(f"\nDuration: {duration}")
    print("\nPhase Results:")
    ...
    if final_path.exists():
        ...
        print(f"\nFinal Output: {len(df)} schools, {len(df.columns)} columns")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NRW Secondary School Data Asset Builder Orchestrator"
    )
    parser.add_argument(
        "--phases", type=str,
