...
Results cached per schulnummer to data_leipzig/cache/website_metadata/
...
# --------------------------------------------------------------------------
# Description generation (Phase B)
# --------------------------------------------------------------------------
...
def _get_school_info(row) -> dict:
    ...
    return {
        ...
        "plz": str(row.get("plz", "")),
        "stadt": str(row.get("stadt", row.get("ort", "Leipzig"))),
        "schultyp": str(row.get("schultyp", row.get("school_type", ""))),
...
def enrich_schools() -> pd.DataFrame:
    """
    Enrich Leipzig schools with metadata and descriptions scraped from school websites.
    ...
    """
    logger.info("Enriching Leipzig schools with website metadata + descriptions...")

    # Find input file (most enriched first)
    input_file = None
    ...
    if not input_file:
        raise FileNotFoundError(
            "No input file found for Leipzig website metadata enrichment. "
            "Checked: " + ", ".join(f.name for f in INPUT_FALLBACKS)
        )
    ...
    # Stats
    stats = {
        "meta_api": 0, "meta_cache": 0, "desc_api": 0, "desc_cache": 0,
    ...
    for i, idx in enumerate(website_indices):
        ...
        if snr in meta_cache:
            ...
            data = scrape_school_metadata(
                ...
                info["schultyp"], info["traegerschaft"],
            )
            if data:
                meta_cache[snr] = data
            else:
    ...
    _save_cache(meta_cache, WEBSITE_CACHE)
    logger.info(
        f"  Metadata phase complete: {stats['meta_enriched']} enriched, "
    ...
    _save_cache(desc_cache, DESCRIPTION_CACHE)
    logger.info(
        f"  Description phase complete: {stats['desc_enriched']} enriched, "
        f"{stats['desc_api']} API calls, {stats['desc_cache']} cache hits"
    )

    # Save output
    INTERMEDIATE_DIR.mkdir(parents=True, exist_ok=True)
    output_file = INTERMEDIATE_DIR / "leipzig_schools_with_metadata.csv"
    df.to_csv(output_file, index=False, encoding="utf-8-sig")
    logger.info(f"  Saved: {output_file}")

    # Summary
    print(f"\n{'=' * 70}")
    print("LEIPZIG WEBSITE METADATA ENRICHMENT - COMPLETE")
    print(f"{'=' * 70}")
...
def main():
    """Main entry point."""
    logger.info("=" * 60)
    logger.info("Starting Leipzig Website Metadata Enrichment")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.error(f"Error enriching Leipzig schools: {e}")
        import traceback
        traceback.print_exc()
