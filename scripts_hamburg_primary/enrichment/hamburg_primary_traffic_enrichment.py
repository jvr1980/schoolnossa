#!/usr/bin/env python3
"""
Hamburg Primary School Traffic Data Enrichment
Enriches primary school data with traffic metrics from Hamburg's SensorThings API.

This script:
1. Loads primary school data with coordinates
...
- WFS Verkehrsstaerken: Official DTV (daily traffic volume) values
...
class HamburgPrimaryTrafficEnrichment:
    """Enriches Hamburg primary school data with traffic metrics."""
    ...
    def load_schools(self) -> pd.DataFrame:
        """Load primary school data with coordinates."""
        ...
        possible_files = [
            ...
            INTERMEDIATE_DIR / "hamburg_primary_schools_with_transit.csv",
            DATA_DIR / "raw" / "hamburg_grundschulen.csv"
        ]
                    ...
                    if coords and len(coords) >= 2:
                        ...
                        if dtv is not None:
                            sensors.append({
                                ...
                                'description': f"Zaehlstelle {props.get('zaehlstelle', '')}",
                                'longitude': lon,
                                'latitude': lat,
                                'sensor_type': 'ogc_kfz',
                                'dtv': dtv,
                    ...
                    if coords and len(coords) >= 2:
                        ...
                        if dtv is not None:
                            sensors.append({
                                ...
                                'datastream_id': None,
                                'name': props.get('bezeichnung', 'Unknown'),
                                'description': f"Rad-Zaehlstelle {props.get('zaehlstelle', '')}",
                                'longitude': lon,
                                'latitude': lat,
                                'sensor_type': 'ogc_rad',
                                'dtv': dtv,
    ...
    def get_all_sensors(self) -> pd.DataFrame:
        ...
        cache_file = CACHE_DIR / "hamburg_primary_traffic_sensors.json"
    ...
    def save_output(self, df: pd.DataFrame):
        ...
        # Save CSV
        csv_path = INTERMEDIATE_DIR / "hamburg_primary_schools_with_traffic.csv"
        df.to_csv(csv_path, index=False, encoding='utf-8-sig')
        logger.info(f"Saved: {csv_path}")

        # Save parquet
        parquet_path = INTERMEDIATE_DIR / "hamburg_primary_schools_with_traffic.parquet"
    ...
    def run(self) -> pd.DataFrame:
        ...
        logger.info("="*60)
        logger.info("Starting Hamburg Primary School Traffic Data Enrichment")
        logger.info("="*60)
    ...
    def print_summary(self, df: pd.DataFrame):
        ...
        print("\n" + "="*70)
        print("HAMBURG PRIMARY SCHOOL TRAFFIC ENRICHMENT - COMPLETE")
        print("="*70)
...
def main():
    """Main entry point."""
    enricher = HamburgPrimaryTrafficEnrichment()
    enricher.run()
