#!/usr/bin/env python3
"""
Transform Hamburg primary school data to match Berlin schema exactly.

This script takes the Hamburg primary school master table and transforms it to have
...
HAMBURG_CSV = DATA_DIR / "final" / "hamburg_primary_school_master_table_final.csv"
HAMBURG_EMBEDDINGS = DATA_DIR / "final" / "hamburg_primary_school_master_table_final_with_embeddings.parquet"
...
def transform_hamburg_primary_to_berlin_schema():
    """Transform Hamburg primary school data to match Berlin schema."""

    print("Loading data...")
    # Read from CSV (has all enriched columns)
    ...
    print(f"Hamburg primary: {len(hamburg)} schools, {len(hamburg.columns)} columns")
    ...
    if 'lehrer_estimated' in hamburg.columns:
        ...
        print(f"  Mapped lehrer_estimated -> lehrer_2024_25 ({count} schools)")
    ...
    print(f"  Columns from Hamburg primary data: {populated}/{len(berlin_columns)}")
    ...
    key_cols = ['schulnummer', 'schulname', 'latitude', 'longitude', 'bezirk',
                ...
                'lehrer_2024_25', 'schueler_2024_25',
    ...
    for col in key_cols:
        if col in output.columns:
            ...
            status = "+" if pct > 50 else "o" if pct > 0 else "-"
    ...
    output_csv = OUTPUT_DIR / "hamburg_primary_school_master_table_berlin_schema.csv"
    ...
    print("=" * 60)
    print(f"Input:  {len(hamburg)} Hamburg primary schools, {len(hamburg.columns)} columns")
...
if __name__ == "__main__":
    transform_hamburg_primary_to_berlin_schema()
