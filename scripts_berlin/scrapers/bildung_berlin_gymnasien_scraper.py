#!/usr/bin/env python3
"""
Scraper for Gymnasien schools from bildung.berlin.de

This script scrapes school data from the Berlin education portal, collecting:
- School name
- School code (Schulnummer) - extracted from school name suffix (format: XXY##, e.g., 01Y01)
...
Output: bildung_berlin_gymnasien.csv and bildung_berlin_gymnasien.xlsx
"""
...
SCHULART_ISS = "41"  # Integrierte Sekundarschule
SCHULART_GYMNASIUM = "14"  # Gymnasien
...
def get_session_with_search(schulart_code: str) -> tuple[requests.Session, list[dict]]:
    ...
    # Submit search
    logger.info(f"Submitting search for Schulart code: {schulart_code}")
...
def scrape_school_detail(session: requests.Session, school_url: str, school_name: str, schulart: str = 'Gymnasium') -> dict:
    """
    ...
    try:
        ...
        # ID: ContentPlaceHolderMenuListe_lblSchulname contains "School Name - XXK##"
        schulname_el = soup.find(id='ContentPlaceHolderMenuListe_lblSchulname')
        if schulname_el:
            full_name = schulname_el.get_text(strip=True)
            # Extract school code (format: ##X## or ##S##, e.g., 10K15, 01Y01, 01S06)
...
def scrape_all_gymnasien() -> pd.DataFrame:
    """
    Scrape all Gymnasium schools from bildung.berlin.de
    """
    # Get session and school list
    session, schools = get_session_with_search(SCHULART_GYMNASIUM)

    if not schools:
        logger.error("No schools found!")
        return pd.DataFrame()

    logger.info(f"Starting to scrape {len(schools)} schools...")
...
def main():
    """Main entry point"""
    logger.info("Starting bildung.berlin.de Gymnasien scraper...")

    df = scrape_all_gymnasien()
    ...
    csv_path = RAW_DIR / 'bildung_berlin_gymnasien.csv'
    xlsx_path = RAW_DIR / 'bildung_berlin_gymnasien.xlsx'
    ...
    # Show summary
    print("\n" + "="*60)
    print(f"SUMMARY: {len(df)} Gymnasium schools scraped")
    print("="*60)
