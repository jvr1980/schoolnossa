#!/usr/bin/env python3
"""
Scraper for Integrierte Sekundarschule (ISS) schools from bildung.berlin.de

This script scrapes school data from the Berlin education portal, collecting:
- School name
- School code (Schulnummer) - extracted from school name suffix
...
Output: bildung_berlin_iss.csv and bildung_berlin_iss.xlsx
"""
...
def scrape_school_detail(session: requests.Session, school_url: str, school_name: str, schulart: str = 'Integrierte Sekundarschule') -> dict:
    """
...
def scrape_all_iss_schools() -> pd.DataFrame:
    """
    Scrape all Integrierte Sekundarschule schools from bildung.berlin.de
    """
    # Get session and school list
    session, schools = get_session_with_search(SCHULART_ISS)
...
def main():
    """Main entry point"""
    logger.info("Starting bildung.berlin.de ISS scraper...")

    df = scrape_all_iss_schools()
    ...
    # Show summary
    print("\n" + "="*60)
    print(f"SUMMARY: {len(df)} Integrierte Sekundarschule schools scraped")
    print("="*60)
