#!/usr/bin/env python3
"""
Enriches school data with traffic metrics from Berlin traffic sensors.
...
def geocode_address_nominatim(address):
    ...
    headers = {
        "User-Agent": "SchoolNossa/1.0 (school data enrichment project)"
    }
    params = {
        "q": address,
        "format": "json",
...
def load_geocode_cache():
    ...
    if os.path.exists(GEOCODED_CACHE):
...
def extract_sensor_locations():
    ...
    if os.path.exists(TELRAAM_GEOJSON):
    ...
    if os.path.exists(ECOCOUNTER_GEOJSON):
...
def find_nearest_sensors(df_schools, df_sensors, radius_meters=SEARCH_RADIUS_METERS):
    ...
    print(f"  - Avg sensors per matched school: {total_matches/max(schools_with_sensors,1):.1f}")
...
def load_traffic_data(year=2024):
    ...
    for filename in os.listdir(TELRAAM_DATA_DIR):
        if f"_{year}_" in filename:
            telraam_files.append(os.path.join(TELRAAM_DATA_DIR, filename))
    ...
    for year_check in [year, 2022, 2021, 2020]:
        for filename in os.listdir(ECOCOUNTER_DATA_DIR):
            if f"_{year_check}_" in filename:
                ecocounter_files.append(os.path.join(ECOCOUNTER_DATA_DIR, filename))
        if ecocounter_files:
...
def main():
    """Main function to enrich schools with traffic data."""
    print("="*70)
    print("ENRICHING SCHOOL DATA WITH TRAFFIC METRICS")
    print("="*70)
