#!/usr/bin/env python3
"""
Hamburg School Master Data Scraper
...
3. Filters for Stadtteilschulen and Gymnasien only
4. Extracts coordinates from GeoJSON
5. Outputs: hamburg_schools_raw.csv
...
# School types we want to keep (secondary schools)
SECONDARY_SCHOOL_TYPES = [
    'Stadtteilschule',
    'Gymnasium',
    'Stadtteilschule, Gymnasium',  # Combined schools
]
...
def filter_secondary_schools(df: pd.DataFrame) -> pd.DataFrame:
    """Filter DataFrame to keep only Stadtteilschulen and Gymnasien."""
    logger.info("Filtering for secondary schools (Stadtteilschule & Gymnasium)...")
    ...
    # Create filter mask
    mask = df[type_col].str.lower().str.contains('stadtteilschule|gymnasium', na=False, regex=True)
...
def clean_and_enrich_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Add school_type standardized column
    if 'schulart' in df.columns:
        def categorize_school_type(art):
            if pd.isna(art):
                return 'Unknown'
            art_lower = str(art).lower()
            if 'gymnasium' in art_lower and 'stadtteilschule' in art_lower:
                return 'Stadtteilschule_Gymnasium'  # Combined schools
            elif 'gymnasium' in art_lower:
                return 'Gymnasium'
            elif 'stadtteilschule' in art_lower:
                return 'Stadtteilschule'
            else:
                return 'Other'
...
def save_outputs(df: pd.DataFrame):
    ...
    # Save raw CSV
    raw_csv_path = RAW_DIR / "hamburg_schools_raw.csv"
    df.to_csv(raw_csv_path, index=False, encoding='utf-8-sig')
    logger.info(f"Saved: {raw_csv_path}")

    # Save as parquet for efficient storage
    raw_parquet_path = RAW_DIR / "hamburg_schools_raw.parquet"
    df.to_parquet(raw_parquet_path, index=False)
    logger.info(f"Saved: {raw_parquet_path}")

    # Also save secondary schools only
    if 'school_type' in df.columns:
        secondary_only = df[df['school_type'].isin(['Gymnasium', 'Stadtteilschule', 'Stadtteilschule_Gymnasium'])]
        secondary_csv_path = RAW_DIR / "hamburg_secondary_schools.csv"
        secondary_only.to_csv(secondary_csv_path, index=False, encoding='utf-8-sig')
        logger.info(f"Saved: {secondary_csv_path} ({len(secondary_only)} schools)")
...
def print_summary(df: pd.DataFrame):
    ...
    print("\n" + "="*70)
    print("HAMBURG SCHOOL MASTER DATA SCRAPER - PHASE 1 COMPLETE")
    print("="*70)
...
def main():
    """Main function to run the Hamburg school master data scraper."""
    logger.info("="*60)
    logger.info("Starting Hamburg School Master Data Scraper (Phase 1)")
    logger.info("="*60)
    ...
    try:
        ...
        secondary_df = filter_secondary_schools(schools_df)

        # Save outputs
        save_outputs(secondary_df)

        # Print summary
        print_summary(secondary_df)
