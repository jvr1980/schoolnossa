#!/usr/bin/env python3
"""
Hamburg Crime Data Enrichment
Enriches school data with crime statistics from Hamburg PKS (Polizeiliche Kriminalstatistik).

This script:
1. Downloads the PKS Stadtteilatlas PDF from Hamburg Police
2. Extracts crime statistics by Stadtteil (neighborhood)
3. Matches schools to their Stadtteil
...
HAMBURG_BEZIRKE = {
    ...
    'Altona': [
        'Altona-Altstadt', 'Altona-Nord', 'Ottensen', 'Bahrenfeld', 'Groß Flottbek',
    ...
    'Harburg': [
        ...
        'Sinstorf', 'Marmstorf', 'Eißendorf', 'Heimfeld', 'Moorburg', 'Altenwerder',
...
class HamburgCrimeEnrichment:
    """Enriches Hamburg school data with crime statistics."""
    ...
    def get_fallback_crime_data(self) -> pd.DataFrame:
        ...
        # These are approximate values per 1000 residents (Häufigkeitszahl)
        # Source: https://www.hamburg.de/politik-und-verwaltung/behoerden/behoerde-fuer-inneres-und-sport/presseservice/pressemeldungen/kriminalstatistik-2024-1021038

        # Bezirk-level data (official 2024 figures, approximate)
        bezirk_data = {
    ...
    def merge_crime_data(self, df_schools: pd.DataFrame, df_crime: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            ...
            # Always add Bezirk-level aggregate data for context (Häufigkeitszahl)
            if bezirk:
    ...
    def save_output(self, df: pd.DataFrame):
        ...
        # Save CSV
        csv_path = INTERMEDIATE_DIR / "hamburg_schools_with_crime.csv"
        df.to_csv(csv_path, index=False, encoding='utf-8-sig')
        logger.info(f"Saved: {csv_path}")

        # Save parquet
        parquet_path = INTERMEDIATE_DIR / "hamburg_schools_with_crime.parquet"
    ...
    def run(self) -> pd.DataFrame:
        ...
        logger.info("="*60)
        logger.info("Starting Hamburg Crime Data Enrichment")
        logger.info("="*60)
        ...
        if not df_crime.empty:
            crime_cache_path = CACHE_DIR / "hamburg_crime_statistics.csv"
    ...
    def print_summary(self, df: pd.DataFrame):
        ...
        print("\n" + "="*70)
        print("HAMBURG CRIME ENRICHMENT - COMPLETE")
        print("="*70)
        ...
        coverage = {
            ...
            'crime_haeufigkeitszahl_2024': 'Häufigkeitszahl 2024 (Bezirk)',
        ...
        if 'crime_bezirk' in df.columns and df['crime_bezirk'].notna().any():
            print(f"\nSchools by Bezirk (crime data available):")
...
def main():
    """Main entry point."""
    enricher = HamburgCrimeEnrichment()
    enricher.run()
