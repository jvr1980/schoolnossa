#!/usr/bin/env python3
"""
Hamburg HVV Transit Enrichment
Phase 6: Calculate transit accessibility for each school
...
2. Calculates distance from each school to nearest transit stops
3. Computes transit accessibility scores
4. Enriches the school master table with transit data
...
def classify_transit_type(stop_name: str, stop_type: str, properties: dict) -> str:
    ...
    if 'u-bahn' in name_lower or 'u-bahn' in type_lower or 'u-bahn' in all_props:
        ...
        return 'Bus'
    elif 'fähr' in name_lower or 'ferry' in type_lower:
        return 'Fähre'
...
def save_outputs(schools_df: pd.DataFrame, stops_df: pd.DataFrame):
    ...
    schools_path = INTERMEDIATE_DIR / "hamburg_schools_with_transit.csv"
    schools_df.to_csv(schools_path, index=False, encoding='utf-8-sig')
    logger.info(f"Saved: {schools_path}")

    # Save parquet
    parquet_path = INTERMEDIATE_DIR / "hamburg_schools_with_transit.parquet"
...
def print_summary(schools_df: pd.DataFrame):
    ...
    print("\n" + "="*70)
    print("HAMBURG HVV TRANSIT ENRICHMENT - PHASE 6 COMPLETE")
    print("="*70)

    print(f"\nTotal schools processed: {len(schools_df)}")
...
def main():
    """Main function to run transit enrichment."""
    logger.info("="*60)
    logger.info("Starting Hamburg HVV Transit Enrichment (Phase 6)")
    logger.info("="*60)
    ...
    try:
        ...
        if not schools_path.exists():
            logger.error("No school data found. Run Phase 1 first.")
            return None

        schools_df = pd.read_csv(schools_path)
        logger.info(f"Loaded {len(schools_df)} schools from {schools_path}")
