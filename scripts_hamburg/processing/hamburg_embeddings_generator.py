#!/usr/bin/env python3
"""
Hamburg School Embeddings Generator
Phase 8: Generates text embeddings and creates the final master parquet file.
...
5. Outputs: hamburg_school_master_table_final_with_embeddings.parquet
...
def load_master_table() -> pd.DataFrame:
    ...
    if parquet_path.exists():
        ...
        raise FileNotFoundError(f"No master table found in {FINAL_DIR}")
...
def create_school_description(row: pd.Series) -> str:
    """Create a text description for a school based on its data."""
    parts = []

    # School name and type
    name = row.get('schulname', 'Unknown School')
    school_type = row.get('school_type', row.get('schulart', ''))
    parts.append(f"{name} ist ein {school_type} in Hamburg.")
    ...
    if pd.notna(schueler) and schueler > 0:
        parts.append(f"Etwa {int(schueler)} Schüler besuchen diese Schule.")
    ...
    abitur_2024 = row.get('abitur_durchschnitt_2024')
    abitur_2023 = row.get('abitur_durchschnitt_2023')
    if pd.notna(abitur_2024):
        parts.append(f"Abitur-Durchschnitt 2024: {abitur_2024:.2f}.")
    elif pd.notna(abitur_2023):
        parts.append(f"Abitur-Durchschnitt 2023: {abitur_2023:.2f}.")

    # Special programs
    profil = row.get('oberstufenprofil', '')
    if pd.notna(profil) and profil:
        parts.append(f"Oberstufenprofile: {profil}.")
    ...
    if pd.notna(traeger) and traeger:
        ...
        elif 'staatlich' in str(traeger).lower() or 'öffentlich' in str(traeger).lower():
            parts.append("Die Schule ist in staatlicher Trägerschaft.")
...
def save_final_output(df: pd.DataFrame):
    ...
    parquet_path = FINAL_DIR / "hamburg_school_master_table_final_with_embeddings.parquet"
    df.to_parquet(parquet_path, index=False)
    logger.info(f"Saved: {parquet_path}")

    # Also save CSV (without embeddings for readability)
    csv_df = df.drop(columns=['embedding'], errors='ignore')
    csv_path = FINAL_DIR / "hamburg_school_master_table_final.csv"
...
def print_summary(df: pd.DataFrame):
    """Print final summary."""
    print("\n" + "="*70)
    print("HAMBURG SCHOOL MASTER TABLE - FINAL OUTPUT")
    print("="*70)
    ...
    print(f"\nOutput files:")
    print(f"  - {FINAL_DIR / 'hamburg_school_master_table_final_with_embeddings.parquet'}")
    print(f"  - {FINAL_DIR / 'hamburg_school_master_table_final.csv'}")

    print("\n" + "="*70)


def main():
    """Main function to generate embeddings and create final output."""
    logger.info("="*60)
    logger.info("Starting Hamburg School Embeddings Generator (Phase 8)")
    logger.info("="*60)
